...
def get_tree(root_file):
    """
    Select first usable TTree from ROOT file.
    """
    candidate_names = [
        "events",
        "Events",
...
def palette():
    """
    Stable high-contrast jet-color palette.
    """
    return [
        "#e41a1c",  # red
        "#0047ff",  # blue
...
def colors_from_labels(labels):
    """
    Assign one color to each jet label.
    """
    cols = palette()
    output = []
...
def count_jets(labels):
    """
    Count unique non-negative jet labels.
    """
...
def leading_jet_energy(E, labels):
    """
    Compute leading jet energy from particle energies grouped by jet label.
    """
...
def apply_3d_style(ax):
    """
    Matplotlib 3D pane convention:

      ax.xaxis.pane -> side wall
      ax.yaxis.pane -> side wall
      ax.zaxis.pane -> floor / base plane

    Therefore, for a grey base/floor, the important setting is zaxis.pane.
    """

    ax.set_facecolor("white")

    # --------------------------------------------------------
    # Side panes: almost white
    # --------------------------------------------------------
    ax.xaxis.pane.set_facecolor((0.98, 0.98, 0.98, 1.0))
    ax.yaxis.pane.set_facecolor((0.98, 0.98, 0.98, 1.0))

    # --------------------------------------------------------
    # Floor / base plane: grey
    # --------------------------------------------------------
...
def read_flat_particle_tree(input_file):
    """
    Read flat particle-level ROOT tree.
    ...
    data = {
        ...
        "durham": np.asarray(arrays["durham_jet_id"], dtype=int),
        "antikt": np.asarray(arrays["antikt_jet_id"], dtype=int),
    }
...
def build_event(data, event_id=None):
    """
    Build one event dictionary from flat particle-level arrays.
    """
    ...
    if event_id is None:
        ...
        for eid in all_ids:
            ...
            pt = data["pt"][mask]
            durham = data["durham"][mask]
            antikt = data["antikt"][mask]

            n_particles = np.sum(pt > MIN_PT_DISPLAY)
            n_durham = count_jets(durham)
            n_antikt = count_jets(antikt)
            lead_pt = np.max(pt) if len(pt) > 0 else 0.0

            score = (
                1.00 * min(n_particles, 150)
                + 8.00 * min(n_durham, 8)
                + 3.00 * min(n_antikt, 16)
                + 3.00 * lead_pt
            )
    ...
    for key in ["pt", "phi", "E", "y", "durham", "antikt"]:
...
def downselect(ev, labels):
    """
    Keep highest-pT particles if there are too many particles.
    """
...
def plot_one_event(ev, algorithm_key, title, output_base):
    """
    Draw one prism-style event display.
    """
  ...
  # Native z-label is unreliable in mplot3d with tight bounding boxes.
    # Put the z-axis label manually at the upper end of the z-axis, not inside the plot.
    ax.set_zlabel("")
...
def main():
    parser = argparse.ArgumentParser(
        description="Improved FCC-ee prism-style event display."
    )
    ...
    print("----------------------------------------")
    print("FCC-ee improved prism event display")
    print("----------------------------------------")
    ...
    n_durham = count_jets(ev["durham"])
    n_antikt = count_jets(ev["antikt"])

    elead_durham = leading_jet_energy(ev["E"], ev["durham"])
    elead_antikt = leading_jet_energy(ev["E"], ev["antikt"])

    print("Selected event summary")
    print("----------------------------------------")
    print(f"event_id               = {ev['event_id']}")
    print(f"nParticles             = {len(ev['pt'])}")
    print(f"nJets_durham           = {n_durham}")
    print(f"nJets_antikt           = {n_antikt}")
    print(f"Elead_durham [GeV]     = {elead_durham:.4f}")
    print(f"Elead_antikt [GeV]     = {elead_antikt:.4f}")
    print("----------------------------------------")

    durham_base = os.path.join(output_dir, "event_display_durham_prism")
    antikt_base = os.path.join(output_dir, "event_display_antikt_prism")

    durham_png, durham_pdf, _, _ = plot_one_event(
        ev=ev,
        algorithm_key="durham",
        title=r"Durham $e^+e^-$ $k_T$",
        output_base=durham_base,
    )

    antikt_png, antikt_pdf, _, _ = plot_one_event(
        ev=ev,
        algorithm_key="antikt",
        title=r"anti-$k_T$-like",
        output_base=antikt_base,
    )
